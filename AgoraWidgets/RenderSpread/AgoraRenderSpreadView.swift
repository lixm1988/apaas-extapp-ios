...
class AgoraRenderSpreadView: AgoraBaseUIView {
    ...
    private var renderInfo: AgoraSpreadViewInfo? {
        didSet {
            // name
    ...
    public func updateRenderInfo(renderInfo: AgoraSpreadViewInfo) {
...
private extension AgoraRenderSpreadView {
    func createConstrains() {
        ...
        micView.mas_makeConstraints { make in
            make?.left.equalTo()(AgoraWidgetsFit.scale(2))
            make?.bottom.equalTo()(AgoraWidgetsFit.scale(-2))
            make?.width.height().equalTo()(AgoraWidgetsFit.scale(14))
        }
        volumeView.mas_makeConstraints { make in
            make?.width.equalTo()(AgoraWidgetsFit.scale(8.4))
            make?.height.equalTo()(AgoraWidgetsFit.scale(32))
            make?.centerX.equalTo()(micView)
            make?.bottom.equalTo()(micView.mas_top)?.offset()(AgoraWidgetsFit.scale(-4))
        }
        nameLabel.mas_makeConstraints { make in
            make?.centerY.equalTo()(micView)
            make?.left.equalTo()(micView.mas_right)?.offset()(AgoraWidgetsFit.scale(2))
    ...
    func updateMic() {
        ...
        case .available:
            ...
            micView.image = GetWidgetImage(object: self,
                                           imageName)
            volumeView.isHidden = !value.enableAudio
