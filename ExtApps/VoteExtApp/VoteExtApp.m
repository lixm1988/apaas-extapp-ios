...
- (void)initBaseViews {
    ...
    self.titleHeight = 30;
    {
        ...
        labelTitle.text = [@"Vote_title" ag_localizedIn:@"AgoraExtApps"];
...
- (void)initVoteUI: (BOOL)isResult{
    ...
    
    {
        ...
        if (isResult) {
            ...
            [self.voteBtn setTitle:[@"Vote_submit"ag_localizedIn:@"AgoraExtApps"]
...
- (void)refreshTypeVoteTxt {
    if (self.typeLabel) {
        self.typeLabel.text = ((1 == self.currentVoteType) ? [@"Option_mul" ag_localizedIn:@"AgoraExtApps"] : [@"Option_sing" ag_localizedIn:@"AgoraExtApps"]);
    }
}
